...
UPDATE_CHECK_URL = "https://raw.githubusercontent.com/Thisisfenix/Mascota_saltarina/main/HolaGissel/version.json"
UPDATE_DOWNLOAD_URL = "https://github.com/Thisisfenix/Mascota_saltarina/releases/latest/download/HolaGissel.zip"
...
def verificar_actualizacion_disponible():
    """Verifica si hay actualizacion sin bloquear"""
    try:
        response = requests.get(UPDATE_CHECK_URL, timeout=3)
        if response.status_code == 200:
            data = response.json()
            if data.get('version', '0.0.0') > VERSION_ACTUAL:
                return data
    except:
        pass
    return None

def mostrar_splash_actualizacion():
    ...
    resultado = {'actualizar': False, 'info': None}
    
    def verificar():
        info = verificar_actualizacion_disponible()
        if info:
            status_label.config(text=f"Nueva version {info['version']} disponible!")
            splash.after(1000, lambda: preguntar_actualizar(splash, info, resultado))
        else:
            status_label.config(text="Iniciando...")
            splash.after(500, splash.destroy)
    
    def preguntar_actualizar(ventana, info, res):
        ventana.destroy()
        respuesta = messagebox.askyesno(
            "Actualizacion disponible",
            f"Nueva version {info['version']} disponible!\n\nCambios:\n" + 
            "\n".join([f"• {c}" for c in info.get('cambios', [])]) +
...
class ImagenFlotante:
    def __init__(self):
        ...
        try:
            ...
            if os.path.exists(self.icon_path):
                ...
                # Intentar en assets
                self.icon_path = os.path.abspath(os.path.join("assets", "icon.ico"))
                if os.path.exists(self.icon_path):
                    self.root.iconbitmap(self.icon_path)
                else:
                    # Si no existe .ico, intentar con .png
                    icon_png = os.path.abspath(os.path.join("assets", "AnkushCat.png"))
                    if os.path.exists(icon_png):
                        icon_img = Image.open(icon_png)
                        icon_img = icon_img.resize((32, 32), Image.Resampling.LANCZOS)
                        icon_photo = ImageTk.PhotoImage(icon_img)
                        self.root.iconphoto(True, icon_photo)
                        self.icon_photo = icon_photo  # Guardar referencia
    ...
    def crear_panel_control(self):
        ...
        # Navegación con estilo moderno
        nav_frame = tk.Frame(self.root, bg='#1e1b4b', height=50)
    ...
    def cambiar_pagina(self, pagina):
        ...
        for p in ['principal', 'efectos', 'eventos', 'acciones', 'stats', 'juegos']:
    ...
    def crear_pagina_mods(self):
        ...
        if not self.mods_cargados:
            ...
            for mod in self.mods_cargados:
                ...
                mod_var = tk.BooleanVar(value=mod.get('activo', True))
                mod['var'] = mod_var
            ...
            if filename.endswith('.py') and not filename.startswith('__'):
                try:
                    ...
                    if hasattr(mod, 'MOD_INFO'):
                        ...
                        mod_info['module'] = mod
                        mod_info['activo'] = True
                        self.mods_cargados.append(mod_info)
                        
                        if hasattr(mod, 'inicializar'):
                            try:
                                mod.inicializar(self)
                            except Exception as e:
                                error_msg = f"{filename}: Error en inicializar()\n{str(e)}"
    ...
    def tirar_dado(self):
        ...
        def ejecutar_accion():
            ...
            6: ("🎁 Teletransporte", lambda: setattr(self, 'x', random.randint(0, self.ancho_pantalla - self.tamaño)) or setattr(self, 'y', random.randint(0, self.alto_pantalla - self.tamaño)))
        }
    ...
    def dibujar_trail(self):
        ...
        while len(self.trail_ventanas) < num_necesarias:
            ...
            # Crear copia de la imagen para evitar referencias
            try:
                label_trail = tk.Label(trail_win, image=self.imagen, bg='white')
                label_trail.pack()
            except:
                trail_win.destroy()
                continue
    ...
    def cerrar_con_confirmacion(self):
        """Pide confirmación antes de cerrar el programa solo si hay cambios sin guardar"""
...
def descargar_e_instalar_actualizacion(info):
    ...
    tk.Label(ventana, text="🔄 Actualizando Ankush Cat", bg='#1e1e2e', fg='#89b4fa',
            font=('Arial', 14, 'bold')).pack(pady=20)
    
    status = tk.Label(ventana, text="Descargando actualización...", bg='#1e1e2e',
                     fg='#cdd6f4', font=('Arial', 10))
    status.pack(pady=10)
    
    progress = Progressbar(ventana, length=350, mode='determinate')
    progress.pack(pady=20)
    
    porcentaje = tk.Label(ventana, text="0%", bg='#1e1e2e', fg='#a6e3a1',
                         font=('Arial', 12, 'bold'))
    ...
    def actualizar():
        try:
            # Descargar
            status.config(text="Descargando actualización...")
            ventana.update()
            
            response = requests.get(UPDATE_DOWNLOAD_URL, stream=True, timeout=60)
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if chunk:
                        ...
                        if total > 0:
                            ...
                            progress['value'] = pct
                            porcentaje.config(text=f"{int(pct)}%")
                            ventana.update()
            
            # Instalar
            status.config(text="Instalando actualización...")
            progress['value'] = 0
            porcentaje.config(text="Instalando...")
            ventana.update()
            
            # Backup de configs
            backup_items = ["config_mascota.json", "mods", "screenshots"]
            ...
            status.config(text="✅ Actualización completada!")
            progress['value'] = 100
...
if __name__ == "__main__":
    ...
    resultado = mostrar_splash_actualizacion()
    
    if resultado['actualizar'] and resultado['info']:
        descargar_e_instalar_actualizacion(resultado['info'])
